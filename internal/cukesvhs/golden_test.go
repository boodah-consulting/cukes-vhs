...
var _ = Describe("Golden baseline management", func() {
	...
	Describe("SaveBaseline", func() {
		Context("with valid source files", func() {
			It("creates the scenario directory under goldenDir", func() {
				Expect(cukesvhs.SaveBaseline(goldenDir, "My Scenario", asciiSrc, gifSrc)).To(Succeed())
			...
			It("slugifies the scenario name to form the directory path", func() {
				Expect(cukesvhs.SaveBaseline(goldenDir, "Hello World! 123", asciiSrc, gifSrc)).To(Succeed())
			...
			It("handles scenario names with underscores", func() {
				Expect(cukesvhs.SaveBaseline(goldenDir, "capture_event", asciiSrc, gifSrc)).To(Succeed())
		...
		Context("when the ASCII source does not exist", func() {
			It("returns an error", func() {
				err := cukesvhs.SaveBaseline(goldenDir, "Missing", "/nonexistent/output.txt", gifSrc)
		...
		Context("when the GIF source does not exist", func() {
			It("returns an error", func() {
				err := cukesvhs.SaveBaseline(goldenDir, "Missing", asciiSrc, "/nonexistent/output.gif")
				Expect(err).To(HaveOccurred())
	...
	Describe("GetBaseline", func() {
		Context("when a baseline exists for the scenario", func() {
			...
			It("returns the correct ASCII path", func() {
				ascii, exists, err := cukesvhs.GetBaseline(goldenDir, "My Scenario")
				Expect(err).NotTo(HaveOccurred())
			...
			It("returns the correct GIF path via ListBaselines", func() {
				_, exists, err := cukesvhs.GetBaseline(goldenDir, "My Scenario")
				Expect(err).NotTo(HaveOccurred())
				Expect(exists).To(BeTrue())
				baselines, listErr := cukesvhs.ListBaselines(goldenDir)
		...
		Context("when no baseline exists for the scenario", func() {
			It("returns exists=false and no error", func() {
				_, exists, err := cukesvhs.GetBaseline(goldenDir, "Nonexistent Scenario")
				Expect(err).NotTo(HaveOccurred())
				Expect(exists).To(BeFalse())
			})

			It("returns empty ASCII path", func() {
				ascii, _, _ := cukesvhs.GetBaseline(goldenDir, "Nonexistent Scenario")
		...
		Context("when the scenario directory exists but baseline.txt is missing", func() {
			...
			It("returns exists=false and no error", func() {
				_, exists, err := cukesvhs.GetBaseline(goldenDir, "partial scenario")
				Expect(err).NotTo(HaveOccurred())
				Expect(exists).To(BeFalse())
			})
		})
		...
		Context("when the scenario directory exists but baseline.gif is missing", func() {
			...
			It("returns exists=false and no error", func() {
				_, exists, err := cukesvhs.GetBaseline(goldenDir, "partial gif")
				Expect(err).NotTo(HaveOccurred())
				Expect(exists).To(BeFalse())
			})
		})
	...
	Describe("UpdateBaseline", func() {
		Context("when a baseline already exists", func() {
			...
			BeforeEach(func() {
				Expect(cukesvhs.SaveBaseline(goldenDir, "Update Me", asciiSrc, gifSrc)).To(Succeed())
			...
			It("overwrites the ASCII baseline with new content", func() {
				Expect(cukesvhs.UpdateBaseline(goldenDir, "Update Me", newASCII, newGIF)).To(Succeed())
		...
		Context("when no baseline exists yet", func() {
			It("creates the baseline from scratch", func() {
				Expect(cukesvhs.UpdateBaseline(goldenDir, "Brand New", asciiSrc, gifSrc)).To(Succeed())

				_, exists, err := cukesvhs.GetBaseline(goldenDir, "Brand New")
				Expect(err).NotTo(HaveOccurred())
				Expect(exists).To(BeTrue())
			})
		})
		...
		Context("when the source file is missing", func() {
			It("returns an error", func() {
				err := cukesvhs.UpdateBaseline(goldenDir, "Fail", "/nonexistent/file.txt", gifSrc)
				Expect(err).To(HaveOccurred())
			})
		})
	})
	...
	Describe("ListBaselines", func() {
		Context("when the golden dir is empty", func() {
			It("returns an empty slice and no error", func() {
				results, err := cukesvhs.ListBaselines(goldenDir)
				Expect(err).NotTo(HaveOccurred())
		...
		Context("when the golden dir does not exist", func() {
			It("returns an empty slice and no error", func() {
				results, err := cukesvhs.ListBaselines(filepath.Join(goldenDir, "nonexistent"))
				Expect(err).NotTo(HaveOccurred())
				Expect(results).To(BeEmpty())
			})
		})
		...
		Context("when baselines exist", func() {
			BeforeEach(func() {
				Expect(cukesvhs.SaveBaseline(goldenDir, "Scenario Alpha", asciiSrc, gifSrc)).To(Succeed())
				Expect(cukesvhs.SaveBaseline(goldenDir, "Scenario Beta", asciiSrc, gifSrc)).To(Succeed())
			})
