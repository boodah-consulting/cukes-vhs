...
// validatedMsg carries the validation results from the validation stage.
type validatedMsg struct {
	results []vhsgen.ValidationResult
}

// runModel is the Bubble Tea model for the full run pipeline.
// It orchestrates parsing, tape generation, rendering, and validation.
//
// Expected: created via NewRunModel with directory paths and config.
...
// NewRunModel creates a new run model for the full pipeline.
//
// Expected: featuresDir and scenariosDir are readable directories;
// outputDir and goldenDir are writable directories; configSource is
// the path to the config source file; timeoutSec is the render timeout
// in seconds.
// Returns: a runModel ready for use with tea.NewProgram.
// Side effects: none.
func NewRunModel(
	featuresDir, scenariosDir, outputDir, goldenDir, configSource string,
	timeoutSec int,
) runModel {
	return runModel{
...
// Update implements tea.Model. Routes messages through the run pipeline
// stages: parse → generate (one-by-one) → render → validate.
//
...
func (m runModel) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case renderedMsg:
		...
		// Move to validation.
		m.progress.UpdateStage(Validating)
	...
	case validatedMsg:
		for i := range msg.results {
			...
			case vhsgen.ValidationPass:
				m.progress.UpdateScenario(r.Scenario, Done)
			case vhsgen.ValidationFail:
				m.progress.UpdateScenario(r.Scenario, Failed)
...
func (m runModel) parseCmd() tea.Msg {
...
func (m runModel) generateOneCmd(idx int) tea.Cmd {
...
func (m runModel) renderCmd() tea.Msg {
...
// validateCmd validates rendered output against golden baselines.
//
// Expected: m.goldenDir and m.outputDir are valid directories.
// Returns: a validatedMsg with results, or an errMsg.
// Side effects: reads and potentially writes golden baseline files.
func (m runModel) validateCmd() tea.Msg {
	results, err := vhsgen.ValidateAll(m.goldenDir, m.outputDir)
	if err != nil {
		return errMsg{fmt.Errorf("validating output: %w", err)}
	}

	return validatedMsg{results: results}
}
...
// Side effects: none.
func (m runModel) findNextPending() int {
...
// RunWithTTY runs the full run pipeline with TUI.
//
...
// spawns VHS child processes.
func RunWithTTY(
	featuresDir, scenariosDir, outputDir, goldenDir, configSource string,
	timeoutSec int,
) error {
	model := NewRunModel(
		featuresDir, scenariosDir, outputDir, goldenDir,
