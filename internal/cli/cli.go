...
func printUsageTo(out io.Writer) {
	...
	fmt.Fprintln(out, "  cukes-vhs init [flags]              Initialise config file for customisation")
	fmt.Fprintln(out, "  cukes-vhs list [flags]              List scenarios and their translatability")
	fmt.Fprintln(out, "  cukes-vhs generate [flags]          Generate VHS tape files from scenarios")
	fmt.Fprintln(out, "  cukes-vhs run [flags]               Full pipeline: generate → render → validate")
	fmt.Fprintln(out, "  cukes-vhs update-baseline [flags]   Accept current output as new golden baseline")
	fmt.Fprintln(out, "")
	printInitFlags(out)
	printListFlags(out)
...
func printExamples(out io.Writer) {
	...
	fmt.Fprintln(out, "  cukes-vhs init                              # Create default config")
	fmt.Fprintln(out, "  cukes-vhs init --output my-config/         # Custom output directory")
	fmt.Fprintln(out, "  cukes-vhs list --features features/ --scenarios-dir demos/scenarios/")
	fmt.Fprintln(out, "  cukes-vhs list --json")
	fmt.Fprintln(out, "  cukes-vhs list --count")
	fmt.Fprintln(out, "  cukes-vhs list --steps")
	fmt.Fprintln(out, "  cukes-vhs list --steps --json")
	fmt.Fprintln(out, "  cukes-vhs generate --all --features features/ --scenarios-dir demos/scenarios/ --output /tmp/tapes/")
	fmt.Fprintln(out, "  cukes-vhs generate --feature onboarding --output /tmp/test/")
	fmt.Fprintln(out, "  cukes-vhs run --all --output /tmp/tapes/ --golden demos/golden/")
	fmt.Fprintln(out, "  cukes-vhs run --feature onboarding --output /tmp/tapes/")
	fmt.Fprintln(out, "  cukes-vhs update-baseline --all --output /tmp/tapes/ --golden demos/golden/")
	fmt.Fprintln(out, "  cukes-vhs update-baseline my-scenario --output /tmp/tapes/")
}
